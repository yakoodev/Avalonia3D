...
void main()
{
    ...
    private static string BuildFragmentShaderSource(PbrFeatures features, int maxLights)
    {
        ...
        sb.AppendLine("highp vec3 reflection=ComputeEnvironmentReflection(norm,viewDir,roughness); vec3 iblDiffuse = diffuseColor * max(uIblDiffuseIntensity,0.0) * (1.0-metallic); float metallicReflectionBoost = mix(0.35,1.15,metallic); reflection *= metallicReflectionBoost*(1.0-0.55*clamp(roughness,0.0,1.0));");
    ...
    private static void AppendUniforms(StringBuilder sb, int maxLights)
    {
        ...
        sb.AppendLine("uniform sampler2D uEnvironmentMap; uniform float uReflectionIntensity; uniform float uIblDiffuseIntensity; uniform float uIblSpecularIntensity; uniform float uReflectionContributionClamp; uniform float uAmbientStrengthClamp; uniform int uSeparateEmissiveTarget; uniform float uSeparateEmissiveSurfaceScale; uniform int uHasEnvironmentMap; uniform float uAlpha; uniform float uAlphaCutoff; uniform int uAlphaMode; uniform float uEmissiveIntensity;");
