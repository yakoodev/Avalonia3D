...
void main()
{
    ...
    private static string BuildFragmentShaderSource(PbrFeatures features, int maxLights)
    {
        ...
        sb.AppendLine($"vec3 resultLight=vec3(0.0); float smoothness=clamp(1.0-roughness,0.04,1.0); float shininess=mix(2.0,float(uShininess),smoothness); float ambientStrength=min(max(uAmbientStrength,0.0),max(uAmbientStrengthClamp,0.0)); for(int i=0;i<{maxLights};i++){{ if(i>=uLightCount) break; vec3 toLight=uLightPos[i]-FragPos; float lightDistance=max(length(toLight),0.0001); vec3 lightDir=toLight/lightDistance; float attenuation=1.0/(1.0+lightDistance*lightDistance); vec3 ambient=ambientStrength*uLightColor[i]*attenuation; float diff=max(dot(norm,lightDir),0.0); vec3 diffuse=diff*diffuseColor*uLightColor[i]*attenuation; vec3 reflectDir=reflect(-lightDir,norm); highp float spec=pow(max(dot(viewDir,reflectDir),0.0),shininess); highp vec3 specular=uSpecularStrength*spec*specularColor*uLightColor[i]*attenuation; float shadow=uHasShadowMap==1?ShadowCalculation(FragPosLightSpace,norm,lightDir):0.0; resultLight+=(ambient+(1.0-shadow)*(diffuse+specular))*uIntensity[i]; }} if(uLightCount==0) resultLight=albedo*0.65;");
    ...
    private static void AppendUniforms(StringBuilder sb, int maxLights)
    {
        ...
        sb.AppendLine("uniform sampler2D uEnvironmentMap; uniform float uReflectionIntensity; uniform float uIblDiffuseIntensity; uniform float uIblSpecularIntensity; uniform float uReflectionContributionClamp; uniform float uAmbientStrengthClamp; uniform float uDirectLightContributionClamp; uniform int uSeparateEmissiveTarget; uniform float uSeparateEmissiveSurfaceScale; uniform int uHasEnvironmentMap; uniform float uAlpha; uniform float uAlphaCutoff; uniform int uAlphaMode; uniform float uEmissiveIntensity;");
