...
void main()
{
    ...
    private static string BuildFragmentShaderSource(PbrFeatures features, int maxLights)
    {
        ...
        sb.AppendLine(features.HasFlag(PbrFeatures.ReflectionsIbl)
            ? @"vec3 ComputeEnvironmentReflection(vec3 n, vec3 v, float r){ if(uHasEnvironmentMap==0) return vec3(0.0); vec3 rd=reflect(-v,n); vec2 uv=vec2(atan(rd.z, rd.x)/(2.0*3.14159265)+0.5, acos(clamp(rd.y,-1.0,1.0))/3.14159265); float roughness=clamp(r,0.0,1.0); float maxLod=max(uEnvironmentReflectionMaxLod,0.0); float lod=roughness*roughness*maxLod; vec3 sampled=textureLod(uEnvironmentMap, uv, lod).rgb; sampled=pow(max(sampled, vec3(0.0)), vec3(2.0)); float horizon=clamp(rd.y*0.5+0.5,0.0,1.0); vec3 studioBase=mix(vec3(0.05,0.04,0.035), vec3(0.42,0.32,0.24), horizon); float stripA=pow(clamp(1.0-abs(rd.y-0.28)*7.0,0.0,1.0), 20.0); float stripB=pow(clamp(1.0-abs(rd.y+0.18)*9.5,0.0,1.0), 22.0); float stripC=pow(clamp(1.0-abs(rd.x*0.85+rd.z*0.15)*6.5,0.0,1.0), 18.0); vec3 studioSpec=vec3(1.85)*stripA + vec3(1.35,1.18,1.05)*stripB + vec3(1.05,0.95,0.85)*stripC; sampled=mix(studioBase+studioSpec, sampled, 0.7); float ndotv=max(dot(n,v),0.0); float fresnelBoost=mix(1.0,1.65,pow(1.0-ndotv,5.0)); float grazingBoost=mix(1.0,1.28,1.0-ndotv); return sampled*uReflectionIntensity*uIblSpecularIntensity*fresnelBoost*grazingBoost; }"
        ...
        sb.AppendLine($"vec3 resultLight=vec3(0.0); float smoothness=clamp(1.0-roughness,0.04,1.0); float shininess=mix(2.0,float(uShininess),smoothness); float ambientStrength=min(max(uAmbientStrength,0.0),max(uAmbientStrengthClamp,0.0)); for(int i=0;i<{maxLights};i++){{ if(i>=uLightCount) break; vec3 ambient=ambientStrength*0.35*diffuseColor*uLightColor[i]; vec3 lightDir=normalize(uLightPos[i]-FragPos); float diff=max(dot(norm,lightDir),0.0); vec3 diffuse=diff*diffuseColor*uLightColor[i]; vec3 reflectDir=reflect(-lightDir,norm); highp float spec=pow(max(dot(viewDir,reflectDir),0.0),shininess); highp vec3 specular=uSpecularStrength*spec*specularColor*uLightColor[i]*mix(0.9,1.6,metallic); float shadow=uHasShadowMap==1?ShadowCalculation(FragPosLightSpace,norm,lightDir):0.0; resultLight+=ambient+(1.0-shadow)*(diffuse+specular)*uIntensity[i]; }} if(uLightCount==0){{ float fallbackGloss=clamp(1.0-roughness,0.0,1.0); vec3 fallbackSpecular=specularColor*(0.06+0.24*fallbackGloss); resultLight=diffuseColor*0.04+fallbackSpecular; }}");

        sb.AppendLine("highp vec3 reflection=ComputeEnvironmentReflection(norm,viewDir,roughness); vec3 iblDiffuse = diffuseColor * max(uIblDiffuseIntensity,0.0) * (1.0-metallic); float metallicReflectionBoost = mix(0.45,1.7,metallic); reflection *= metallicReflectionBoost*(1.0-0.35*clamp(roughness,0.0,1.0));");
        ...
        sb.AppendLine($"vec3 totalEmissive = emissive + uEmissionColor; float emissiveSurfaceScale = uSeparateEmissiveTarget==1?clamp(uSeparateEmissiveSurfaceScale,0.0,1.0):1.0; vec3 totalEmissiveForSurface = totalEmissive * emissiveSurfaceScale; vec3 directLightComponent = resultLight; float metallicMask = smoothstep(0.55,0.9,metallic); vec3 iblSpecular = reflection * max(specularColor, vec3(0.04)); iblSpecular *= metallicMask*mix(1.0,1.4,metallic); vec3 iblComponent = min((iblSpecular*mix(1.0,ao,0.2) + iblDiffuse*ao), vec3(max(uReflectionContributionClamp,0.0))); vec3 surfaceResult = directLightComponent + totalEmissiveForSurface + iblComponent + sheenContribution; vec3 debugSurfaceResult = surfaceResult; if(uPbrDebugViewMode=={(int)PbrDebugViewMode.BaseColorOnly}){{ debugSurfaceResult=baseColor.rgb; }} else if(uPbrDebugViewMode=={(int)PbrDebugViewMode.DirectLightOnly}){{ debugSurfaceResult=directLightComponent; }} else if(uPbrDebugViewMode=={(int)PbrDebugViewMode.IblOnly}){{ debugSurfaceResult=iblComponent; }} else if(uPbrDebugViewMode=={(int)PbrDebugViewMode.EmissiveOnly}){{ debugSurfaceResult=totalEmissive; }} else if(uPbrDebugViewMode=={(int)PbrDebugViewMode.AoOnly}){{ debugSurfaceResult=vec3(ao); }} else if(uPbrDebugViewMode=={(int)PbrDebugViewMode.NormalsOnly}){{ debugSurfaceResult=norm*0.5+0.5; }} float sampledAlpha = baseColor.a*uAlpha; if(uAlphaMode==1 && sampledAlpha<uAlphaCutoff){{ discard; }}");

        if (features.HasFlag(PbrFeatures.Transmission))
        {
            sb.AppendLine("float transmissionWeight=0.0; vec3 transmittedLight=surfaceResult; if(uHasTransmission==1 && uPbrDebugViewMode==0){ transmissionWeight=clamp(uTransmissionFactor*transmissionMapSample,0.0,1.0); float mappedThickness=max(uTransmissionThickness*thicknessMapSample,0.0); float attenuationDistance=max(uTransmissionAttenuationDistance,0.0001); float attenuationT=clamp(mappedThickness/attenuationDistance,0.0,1.0); vec3 attenuationTint=mix(vec3(1.0), clamp(uTransmissionAttenuationColor, vec3(0.0), vec3(1.0)), attenuationT); transmittedLight=surfaceResult*attenuationTint;} vec3 finalSurface=uPbrDebugViewMode==0?mix(surfaceResult, transmittedLight, transmissionWeight):debugSurfaceResult; vec3 result=SanitizeHdrColor(finalSurface); float alphaBase=uAlphaMode==2?sampledAlpha:1.0; float transmissionAlpha=sampledAlpha*(1.0-transmissionWeight); float alpha=mix(alphaBase, transmissionAlpha, float(uHasTransmission)); FragColor=vec4(result, alpha); EmissiveColor=vec4(SanitizeHdrColor(totalEmissive), alpha);");
        }
        else
        {
    ...
    private static void AppendUniforms(StringBuilder sb, int maxLights)
    {
        ...
        sb.AppendLine("uniform sampler2D uEnvironmentMap; uniform float uReflectionIntensity; uniform float uIblDiffuseIntensity; uniform float uIblSpecularIntensity; uniform float uReflectionContributionClamp; uniform float uAmbientStrengthClamp; uniform int uSeparateEmissiveTarget; uniform float uSeparateEmissiveSurfaceScale; uniform int uHasEnvironmentMap; uniform float uEnvironmentReflectionMaxLod; uniform float uAlpha; uniform float uAlphaCutoff; uniform int uAlphaMode; uniform float uEmissiveIntensity;");
