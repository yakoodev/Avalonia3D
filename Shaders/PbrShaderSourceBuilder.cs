...
void main()
{
    ...
    private static string BuildFragmentShaderSource(PbrFeatures features, int maxLights)
    {
        ...
        {
            sb.AppendLine("float transmissionWeight=0.0; vec3 transmittedLight=vec3(0.0); if(uHasTransmission==1 && uPbrDebugViewMode==0){ transmissionWeight=clamp(uTransmissionFactor*transmissionMapSample,0.0,1.0); vec3 refractedDir=refract(-viewDir,norm,1.0/max(materialIor,1.0)); float frontLighting=clamp(dot(-refractedDir,norm),0.0,1.0); float mappedThickness=max(uTransmissionThickness*thicknessMapSample,0.0); float thicknessFade=exp(-mappedThickness); float attenuationDistance=max(uTransmissionAttenuationDistance,0.0001); vec3 attenuation=exp(-uTransmissionAttenuationColor*(mappedThickness/attenuationDistance)); vec3 backgroundEstimate=vec3(0.5+0.5*frontLighting)*attenuation*thicknessFade; transmittedLight=backgroundEstimate*transmissionWeight;} vec3 finalSurface=uPbrDebugViewMode==0?mix(surfaceResult, transmittedLight+totalEmissive, transmissionWeight):debugSurfaceResult; vec3 result=SanitizeHdrColor(finalSurface); float alphaBase=uAlphaMode==2?sampledAlpha:1.0; float transmissionAlpha=sampledAlpha*(1.0-transmissionWeight); float alpha=mix(alphaBase, transmissionAlpha, float(uHasTransmission)); FragColor=vec4(result, alpha); EmissiveColor=vec4(SanitizeHdrColor(totalEmissive), alpha);");
        }
        else
        {
