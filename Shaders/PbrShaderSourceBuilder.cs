...
void main()
{
    ...
    private static string BuildFragmentShaderSource(PbrFeatures features, int maxLights)
    {
        ...
        sb.AppendLine("highp vec3 reflection=ComputeEnvironmentReflection(norm,viewDir,roughness); vec3 iblDiffuse = diffuseColor * max(uIblDiffuseIntensity,0.0) * (1.0-metallic); float metallicReflectionBoost = mix(0.35,1.15,metallic); reflection *= metallicReflectionBoost;");
