...
void main()
{
    ...
    private static void AppendUniforms(StringBuilder sb, int maxLights)
    {
        ...
        sb.AppendLine("uniform sampler2D uEnvironmentMap; uniform float uReflectionIntensity; uniform float uIblDiffuseIntensity; uniform float uIblSpecularIntensity; uniform float uReflectionContributionClamp; uniform float uAmbientStrengthClamp; uniform float uDirectLightContributionClamp; uniform int uHasEnvironmentMap; uniform float uAlpha; uniform float uAlphaCutoff; uniform int uAlphaMode; uniform float uEmissiveIntensity;");
