...
- `Program.cs` в `Avalonia3D` и `Avalonia3D.Sandbox`:
...
Ключевой класс: `Model/Scene3D.cs`.
...
`Rendering/GraphicsProfile.cs` определяет:
...
- `Memory/MemoryManager.cs` - мягкая очистка/режимы под крупные ассеты.
- `ModelLoader`:
...
- `Rendering/Diagnostics/MaterialRenderDiagnostics.cs` для PBR-трассировки материалов.
- Python preflight:
